...
import com.zhongruan.android.fingerprint_demo.db.entity.Sb_ip;
...
public class DbServices {
    ...
    public int loadAllbkkssize() {
        return bk_ksDao.loadAll().size();
    }
    ...
    public List<Sfrz_rzjg> selectCCrzjg() {
        return rzjgDao.queryBuilder().where(Sfrz_rzjgDao.Properties.Rzjg_kmno.eq(ks_ccDao.queryBuilder().where(Ks_ccDao.Properties.Cc_extract.eq("1")).list().get(0).getKm_no())).list();
    }
    ...
    public Bk_ks selectKszh(String zh) {
        return bk_ksDao.queryBuilder().where(Bk_ksDao.Properties.Ks_zwh.eq(zh)).list().get(0);
    }
    ...
    public int queryBkKsIsTG(String isRZ) {
        return bk_ksDao.queryBuilder().where(Bk_ksDao.Properties.IsRZ.eq(isRZ)).distinct().list().size();
    }


    public List<Sfrz_rzjg> selectRZJGSB() {
        return rzjgDao.queryBuilder().where(Sfrz_rzjgDao.Properties.Rzjg_kmno.eq(ks_ccDao.queryBuilder().where(Ks_ccDao.Properties.Cc_extract.eq("1")).list().get(0).getKm_no()), Sfrz_rzjgDao.Properties.Rzjg_sb.notEq("1")).list();
    }
    ...
    public Bk_ks selectBKKS(String zjno) {
        return bk_ksDao.queryBuilder().where(Bk_ksDao.Properties.Ks_zjno.eq(zjno)).build().unique();
    }
    ...
    public void saveBkKs(String zjno) {
        List<Bk_ks> bk_ks = bk_ksDao.queryBuilder().where(Bk_ksDao.Properties.Ks_zjno.eq(zjno)).list();
        LogUtil.i(zjno);
        LogUtil.i(bk_ks);
