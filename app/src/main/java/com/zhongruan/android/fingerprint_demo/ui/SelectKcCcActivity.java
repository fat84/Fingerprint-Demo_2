...
import com.zhongruan.android.fingerprint_demo.adapter.SelectKcAdapter;
        ...
        mLlButtons.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!DateUtil.isTime(DateUtil.dateToLong(DateUtil.getNowTime_Millisecond3()), DateUtil.dateToLong(cc.getCc_kssj()), DateUtil.dateToLong(cc.getCc_jssj()))) {
                    ...
                    DbServices.getInstance(getBaseContext()).deleteAllbkks();
                    ...
                    if (selectCcAdapter.getChosenCcList().size() > 0) {
                        ABLSynCallback.call(new ABLSynCallback.BackgroundCall() {
                            ...
                            public Object callback() {
                                ...
                                MyApplication.getDaoInstant(getBaseContext()).getDatabase().execSQL("INSERT INTO " + Bk_ksDao.TABLENAME + " (ks_ksno,ks_xm,ks_xb,ks_zjno,ks_zwh,ks_kcno,ks_kcmc,ks_xp,isRZ) " + " select a.ksno,a.xm,a.xb,a.zjno,a.zw,a.kcno,a.kcmc,b.xp_pic,'0' from " + Bk_ks_tempDao.TABLENAME + " as a," + Bk_ksxpDao.TABLENAME + " as b where  (a.kcno = '" + DbServices.getInstance(getBaseContext()).selectKC().get(0).getKc_no() + "' AND a.ccno = '" + cc.getCc_no() + "'AND a.zjno=b.zjno)");
                                list = DbServices.getInstance(getBaseContext()).loadAllrzkszw();
                                if (list.size() > 0) {
                            ...
                            public void callback(Object obj) {
                                if (((Boolean) obj).booleanValue()) {
                                    for (int i = 0; i < list.size(); i++) {
                                        byte[] bytes = Base64.decode(list.get(i).getZw_feature(), 2);
                                        MyApplication.getYltFingerEngine().importfinger(bytes, list.get(i).getKsid() + "");
                                    }
                                    showProgressDialog(SelectKcCcActivity.this, "正在提取所选场次数据完成", false, 100);
                                    dismissProgressDialog();
                                    new HintDialog(SelectKcCcActivity.this, R.style.dialog, "提取指纹完成，共有" + DbServices.getInstance(getBaseContext()).loadAllbkks().size() + "个考生，有" + DbServices.getInstance(getBaseContext()).loadAllrzkszw().size() + "个指\r\n纹，其中成功" + MyApplication.getYltFingerEngine().getEnrollCount() + "个", new HintDialog.OnCloseListener() {
                                        @Override
                                        public void onClick(Dialog dialog, boolean confirm) {
                                            if (confirm) {
                                                dialog.dismiss();
